...
if (!$disable_automatic_language_changing)
{
  ...
  {
    $lang_file = "lang/lang." . preg_replace("/(\w+)_(\w+)/", "\\1-\\2", $locale);
    if (file_exists($lang_file))
    {
      include $lang_file;
    }
    ...
    {
      $lang_file = "lang/lang.". strtolower(substr($locale,0,2));
      if (file_exists($lang_file))
      {
        include $lang_file;
      }
...
else
{
 ...
 \"".$locale."\" to a Windows locale specifier";
  ...
  else if ($server_os == "unix" || $server_os == "linux" || $server_os == "sunos"
	|| $server_os == "bsd" || $server_os == "macosx")
  {
    if (strlen($locale) == 2)
    {
